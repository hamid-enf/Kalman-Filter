...
kf_status_t kf_kf_reset(kf_kf_t *kf)
{
    ...
    kf_view(&M, kf->P, n, n);  kf_matrix_identity(&M);
    kf_view(&M, kf->Q, n, n);  kf_matrix_zero(&M);
    kf_view(&M, kf->F, n, n);  kf_matrix_identity(&M);
    kf_view(&M, kf->R, m, m);  kf_matrix_zero(&M);
    kf_view(&M, kf->H, m, n);  kf_matrix_zero(&M);
...
kf_status_t kf_kf_predict(kf_kf_t *kf, const kf_real_t *u, kf_real_t dt)
{
    ...
    /* x = F x + u */
    kf_mat_vec_mul(&s[OFF_VEC_N], &Fm, kf->x);
    if (u != NULL) {
    ...
    kf_matrix_mul(&tmp, &Fm, &Pm);            /* tmp = F P           */
    kf_matrix_mul_transpose_b(&Pm, &tmp, &Fm);/* P = (F P) F^T       */
    kf_matrix_add(&Pm, &Pm, &Qm);             /* P += Q              */
...
kf_status_t kf_kf_update(kf_kf_t *kf, const kf_real_t *z)
{
    ...
    /* Innovation y = z - H x */
    kf_mat_vec_mul(&s[OFF_VEC_M], &Hm, kf->x);   /* Hx            */
    ...
    kf_matrix_mul_transpose_b(&Bm, &Pm, &Hm);

    /* S = H B + R  (m x m) */
    kf_view(&Sm, &s[OFF_MM], m, m);
    kf_matrix_mul(&Sm, &Hm, &Bm);
    kf_matrix_add(&Sm, &Sm, &Rm);
    ...
    kf_matrix_transpose(&RHS, &Bm);       /* RHS = B^T          */
    ...
    /* x = x + K y */
    kf_mat_vec_mul(&s[OFF_VEC_N], &Km, &s[OFF_VEC_M]);
    kf_vec_add(kf->x, kf->x, &s[OFF_VEC_N], n);

    /* Covariance update */
    kf_view(&Tm, &s[OFF_NN1], n, n);
    kf_matrix_mul(&Tm, &Km, &Hm);          /* Tm = K H           */
    for (r = 0u; r < n; r++) {
        for (c = 0u; c < n; c++) {
    ...
    kf_matrix_mul(&T2, &Tm, &Pm);            /* T2 = (I-KH) P    */
    kf_matrix_mul_transpose_b(&Pm, &T2, &Tm);/* P = T2 (I-KH)^T  */
    kf_view(&KR, &s[OFF_NM2], n, m);
    kf_matrix_mul(&KR, &Km, &Rm);            /* KR = K R         */
    kf_matrix_mul_transpose_b(&T2, &KR, &Km);/* T2 = K R K^T     */
    kf_matrix_add(&Pm, &Pm, &T2);            /* P += K R K^T     */
#else
...
kf_status_t kf_kf_check(const kf_kf_t *kf)
{
    ...
    for (i = 0u; i < kf->n; i++) {
        if (!kf_isfinite(kf->x[i])) {
            return KF_ERROR_NON_FINITE;
        }
    }
    kf_view(&M, (kf_real_t *)kf->P, kf->n, kf->n);
